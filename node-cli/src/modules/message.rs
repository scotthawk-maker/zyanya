...
impl Handler for Message {
    ...
    async fn handle(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, cmd: &str) -> cli::Result<()> {
        let ctx = ctx.clone().downcast_arc::<ZyanyaNodeCli>()?;
        self.main(ctx, argv, cmd).await.map_err(|e| e.into())
    }
}

impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                (
                    "sign <zyanya_address>",
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, zyanya_address: &str, message: &str) -> Result<()> {
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, zyanya_address: &str, signature: &str, message: &str) -> Result<()> {
        let zyanya_address = Address::try_from(zyanya_address)?;
        if zyanya_address.version != Version::PubKey {
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<ZyanyaNodeCli>, zyanya_address: Address) -> Result<[u8; 32]> {
