...
impl Monitor {
    async fn main(self: Arc<Self>, ctx: &Arc<ZyanyaNodeCli>, _argv: Vec<String>, _cmd: &str) -> Result<()> {
        let max_events = 16;
    ...
    async fn redraw(self: &Arc<Self>, ctx: &Arc<ZyanyaNodeCli>, events: &Arc<Mutex<VecDeque<Box<Events>>>>) -> Result<()> {
        tprint!(ctx, "{}", ClearScreen);
        tprint!(ctx, "{}", Goto(1, 1));
