...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<ZyanyaNodeCli>() {
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<ZyanyaNodeCli>) -> Result<ZyanyadConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();
    ...
    pub async fn handle_event(&self, ctx: &Arc<ZyanyaNodeCli>, event: Event) -> Result<()> {
        let term = ctx.term();
