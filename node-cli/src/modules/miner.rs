...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<ZyanyaNodeCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<ZyanyaNodeCli>) -> Result<()> {
        let root = nw_sys::app::folder();
