...
impl From<&ZyanyaNodeCli> for Arc<Terminal> {
    fn from(ctx: &ZyanyaNodeCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<ZyanyaNodeCli> for ZyanyaNodeCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for ZyanyaNodeCli {
...
impl ZyanyaNodeCli {
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let zyanya_cli = Arc::new(ZyanyaNodeCli {
...
impl cli::Context for ZyanyaNodeCli {
...
pub async fn zyanya_node_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    ZyanyaNodeCli::init();

    let options = Options::new(terminal_options, None);
    let cli = ZyanyaNodeCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Zyanya Node CLI v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
