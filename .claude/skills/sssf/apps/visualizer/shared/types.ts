...
/** GET /info — server identity and liveness: version, uptime, session count. */
export interface InfoResponse {
  /** The visualizer package version, read from package.json at startup. */
  version: string;
  /** Seconds since the server process started (process.uptime()). */
  uptime: number;
  /** Number of session rows in sssf.db. */
  sessions: number;
}

/** GET /health — liveness probe: is the process up, and when did it answer. */
