...
use zyanya_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use zyanya_rpc_service::service::RpcCoreService;
use zyanya_txscript::caches::TxScriptCacheCounters;
use zyanya_utils::git;
...
use zyanya_consensus::{consensus::factory::Factory as ConsensusFactory, pipeline::ProcessingCounters};
...
use zyanya_p2p_flows::{flow_context::FlowContext, service::P2pService};

use itertools::Itertools;
use zyanya_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use zyanya_utxoindex::{api::UtxoIndexProxy, UtxoIndex};
use zyanya_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the zyanyad process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the zyanyad
/// process. (Rusty Zyanya will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("rusty-zyanya");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".rusty-zyanya");
}
...
/// the application folder that contains zyanyad logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                zyanya_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, zyanya_utils::sync::semaphore_module_path()));
            } else {
                zyanya_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the Zyanyad command line to confirm all interactive questions)";
    ...
    let mut meta_db = zyanya_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = zyanya_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from zyanyad DB version
            ...
            || MultiConsensusManagementStore::new(meta_db.clone()).should_upgrade().unwrap())
    {
        ...
        if version == 3 {
            ...
            match active_consensus_dir_name {
                ...
                None => {
                    let msg =
                    "Node database is from a different Zyanyad *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
                    get_user_approval_or_exit(msg, args.yes);
            ...
            info!("Deleting databases from previous Zyanyad version");
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = zyanya_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", zyanya_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", zyanya_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", zyanya_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = zyanya_database::prelude::ConnBuilder::default()
