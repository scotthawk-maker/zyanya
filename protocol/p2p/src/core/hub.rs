...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: ZyanyadMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: ZyanyadMessage) {
    ...
    pub async fn broadcast_many(&self, msgs: Vec<ZyanyadMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();
