...
/// Sign a transaction using schnorr
///
/// F-M-20: deprecated — `sign_with_multiple` maps keys by SEC1 pubkey but looks up
/// by the full script_public_key script, which never matches for the standard
/// pay-to-public-key form. Use [`sign_with_multiple_v2`] instead.
#[deprecated(since = "0.1.0", note = "use sign_with_multiple_v2 instead; key lookup never matches")]
...
/// Sign a transaction using schnorr.
///
/// F-M-20: returns `Result<Signed, Error>` and propagates errors instead of panicking
/// on an invalid private key or a missing UTXO entry.
#[allow(clippy::result_large_err)]
pub fn sign_with_multiple_v2(mut mutable_tx: SignableTransaction, privkeys: &[[u8; 32]]) -> Result<Signed, Error> {
    let mut map = BTreeMap::new();
    for privkey in privkeys {
        let schnorr_key = secp256k1::Keypair::from_seckey_slice(secp256k1::SECP256K1, privkey).map_err(Error::Secp256k1Error)?;
    ...
    for i in 0..mutable_tx.tx.inputs.len() {
        ...
        if let Some(schnorr_key) = map.get(script) {
            let sig_hash = calc_schnorr_signature_hash(&mutable_tx.as_verifiable(), i, SIG_HASH_ALL, &reused_values);
            let msg = secp256k1::Message::from_digest_slice(sig_hash.as_bytes().as_slice()).map_err(Error::Secp256k1Error)?;
            let sig: [u8; 64] = *schnorr_key.sign_schnorr(msg).as_ref();
            // This represents OP_DATA_65 <SIGNATURE+SIGHASH_TYPE> (since signature length is 64 bytes and SIGHASH_TYPE is one byte)
            mutable_tx.tx.inputs[i].signature_script = std::iter::once(65u8).chain(sig).chain([SIG_HASH_ALL.to_u8()]).collect();
        } else {
