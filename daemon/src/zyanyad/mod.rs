...
impl Default for ZyanyadConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<ZyanyadConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: ZyanyadConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no zyanyad path is specified".to_string()));
        }
...
impl Zyanyad {
    pub fn new(args: ZyanyadConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: ZyanyadConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Zyanya node is already running.".to_string()));
            }
        }
        ...
        // let options = ZyanyadOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Zyanyad binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait ZyanyadCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: ZyanyadConfig) -> Result<()>;
