...
from!(item: &zyanya_rpc_core::RpcHeader, protowire::RpcBlockHeader, {
    Self {
...
from!(item: &zyanya_rpc_core::RpcRawHeader, protowire::RpcBlockHeader, {
    Self {
        version: item.version.into(),
        parents: item.parents_by_level.iter().map(protowire::RpcBlockLevelParents::from).collect(),
        hash_merkle_root: item.hash_merkle_root.to_string(),
...
try_from!(item: &protowire::RpcBlockHeader, zyanya_rpc_core::RpcHeader, {
    ...
    let header = Header::new_finalized(
        ...
        item.nonce,
        item.daa_score,
        zyanya_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        item.blue_score,
...
try_from!(item: &protowire::RpcBlockHeader, zyanya_rpc_core::RpcRawHeader, {
    Self {
        ...
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: zyanya_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        blue_score: item.blue_score,
...
mod tests {
    use crate::protowire;
    use zyanya_consensus_core::{block::Block, header::Header};
    use zyanya_rpc_core::{RpcBlock, RpcHash, RpcHeader};
