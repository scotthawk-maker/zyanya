...
pub(crate) async fn create(
    ...
    if let Err(err) = wallet.network_id() {
        tprintln!(ctx);
        tprintln!(ctx, "Before creating a wallet, you need to select a Zyanya network.");
    ...
    let wallet_secret = Secret::from(term.ask(true, "Enter wallet encryption password: ").await?);
    ...
    if !import_with_mnemonic {
        ...
        tpara!(
            ...
            the Zyanya stored in it. Keep your mnemonic safe. Write it down and \
