...
// F-C-13: peer-keyed registry storing the auth token extracted from the
// WebSocket HTTP upgrade request (Authorization header or ?token= query).
// The handshake handler calls `take_handshake_token` to retrieve it.
static HANDSHAKE_TOKENS: std::sync::LazyLock<std::sync::Mutex<std::collections::HashMap<SocketAddr, Option<String>>>> =
    std::sync::LazyLock::new(|| std::sync::Mutex::new(std::collections::HashMap::new()));

/// F-C-13: Retrieve (and remove) the handshake auth token for a peer.
/// Called by the RPC handshake handler to pass the token into the Connection.
pub fn take_handshake_token(peer: &SocketAddr) -> Option<String> {
    HANDSHAKE_TOKENS.lock().unwrap().remove(peer).flatten()
}

/// F-C-13: Callback struct for `accept_hdr_async_with_config` that extracts the
/// bearer token from the HTTP upgrade request and stores it in the peer-keyed registry.
struct TokenExtractor {
    peer: SocketAddr,
}

impl tungstenite::handshake::server::Callback for TokenExtractor {
    fn on_request(
        self,
        ...
        HANDSHAKE_TOKENS.lock().unwrap().insert(self.peer, token);
        Ok(response)
    }
}

/// F-C-13: Extract the bearer token from the HTTP upgrade request headers
/// (`Authorization: Bearer <token>`) or the `?token=<token>` query parameter.
fn extract_token_from_request(request: &tungstenite::handshake::server::Request) -> Option<String> {
    ...
    T: WebSocketHandler + Send + Sync + 'static,
{
    ...
    async fn handle_connection(
        ...
        // F-C-13: Use accept_hdr_async_with_config so we can inspect the HTTP upgrade
        // request and extract the bearer token from headers or query parameter.
        let ws_stream = accept_hdr_async_with_config(stream, TokenExtractor { peer }, config).await?;
